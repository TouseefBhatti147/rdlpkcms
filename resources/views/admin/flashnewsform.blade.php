...
<div class="portlet light bordered">
    ...
    <div class="portlet-body form">
        ...
        @if(isset($AlertEdit))
        <form method="POST" action="{{ url('admin/flashnews/' . $AlertEdit->id) }}" class="form-horizontal"
            enctype="multipart/form-data">
            @method('PUT')
            @else
            <form method="POST" action="{{ url('admin/flashnews') }}" class="form-horizontal"
                enctype="multipart/form-data">
                @endif
                @csrf
                <div class="form-body">
                                ...
                                <div class="input-inline input-large">
                                    <div class="mt-radio-inline">
                                        <label class="mt-radio">
                                            <input type="radio" name="alert_status" value="1"
                                                {{ isset($AlertEdit) ? ($AlertEdit->status == 1 ? 'checked' : '') : 'checked' }}>
                                            Enable <span></span>
                                        </label>
                                        <label class="mt-radio">
                                            <input type="radio" name="alert_status" value="0"
                    ...
                    <div class="row">
                        <div class="form-group @error('alert_description') has-error @enderror">
                            <label for="alert_description" class="control-label col-md-2 col-lg-2">Description</label>
                            <div class="col-md-9">
                                <textarea name="alert_description" id="alert_description" rows="2" cols="88"
                                    class="form-control"
                                    style="resize:none">{{ old('alert_description', $AlertEdit->alert_description ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
...
function changeProfile() {
    // JavaScript function to trigger file input click
}

function removeImage() {
    // JavaScript function to handle image removal
}
</script>
